...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _paste() async {
    ...
    final pos =
        _ctrl.selection.isValid ? _ctrl.selection.baseOffset : _ctrl.text.length;
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            ...
            focusedBorder: OutlineInputBorder(
                borderRadius: BorderRadius.circular(10),
                borderSide: const BorderSide(color: Color(0xFF4A4A8A))),
            contentPadding:
                const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
          ),
          onSubmitted: (v) => Navigator.pop(
              _, v.trim().isNotEmpty ? v.trim() : defaultName),
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () => Navigator.pop(
                _, ctrl.text.trim().isNotEmpty ? ctrl.text.trim() : defaultName),
  ...
  // ── BUILD ─────────────────────────────────────────────────────────────────
  // NOTE: No Container wrapper — return Column directly so Scaffold.body
  // passes tight constraints and Expanded works correctly on all platforms.
  @override
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Expanded(
          child: isEmpty ? _buildHome() : _buildMsgs(),
        ),
        _buildInput(),
      ],
    );
  ...
  // ── HOME (empty state) ────────────────────────────────────────────────────
  // Uses ListView so it definitely works inside Expanded on all platforms.
  Widget _buildHome() {
    return ListView(
            ...
            children: [
              Row(children: [
                Expanded(child: _SuggCard(
                  icon: _suggestions[0].$1,
                  label: _suggestions[0].$2,
                  onTap: () { _ctrl.text = _suggestions[0].$2; _focus.requestFocus(); setState(() {}); },
                )),
                const SizedBox(width: 10),
                Expanded(child: _SuggCard(
                  icon: _suggestions[1].$1,
                  label: _suggestions[1].$2,
              ...
              Row(children: [
                Expanded(child: _SuggCard(
                  icon: _suggestions[2].$1,
                  label: _suggestions[2].$2,
                  onTap: () { _ctrl.text = _suggestions[2].$2; _focus.requestFocus(); setState(() {}); },
                )),
                const SizedBox(width: 10),
                Expanded(child: _SuggCard(
                  icon: _suggestions[3].$1,
                  label: _suggestions[3].$2,
          ...
          children: [
            ...
            Container(
              decoration: BoxDecoration(
                ...
                border:
                    Border.all(color: const Color(0xFF3F3F3F), width: 0.8),
              ),
              child: Row(
                ...
                children: [
                  IconButton(
                    icon: const Icon(Icons.add,
                        color: Color(0xFF8E8EA0), size: 22),
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: const InputDecoration(
                        ...
                        hintStyle: TextStyle(
                            color: Color(0xFF8E8EA0), fontSize: 15),
                  ...
                  IconButton(
                    icon: const Icon(Icons.fullscreen,
                        color: Color(0xFF8E8EA0), size: 20),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            if (msg.text.isNotEmpty)
              Container(
                ...
                child: Text(msg.text,
                    style: const TextStyle(
                        color: Color(0xFFECECEC), fontSize: 15, height: 1.5)),
              ),
          ]),
        ),
      );
          ...
          Expanded(
            child: Padding(
              ...
              child: msg.prompt != null
                  ...
                  : Text(msg.errorText ?? '',
                      style: const TextStyle(
                          color: Color(0xFFAAAAAA), fontSize: 14, height: 1.55)),
            ),
          ),
        ]),
      );
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('ID: ${widget.prompt.id}',
              style: const TextStyle(
                  color: Color(0xFF446644), fontSize: 11)),
            ...
            child: ElevatedButton.icon(
              onPressed: () async {
                await Clipboard.setData(
                    ClipboardData(text: widget.prompt.link));
